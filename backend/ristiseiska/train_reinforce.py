...
def terminal_rank_reward(rank: int) -> float:
    """
    Sharper terminal reward:
      1st matters a lot,
      4th is strongly punished.
    """
    if rank == 1:
        return 1.0
    if rank == 2:
...
def main():
    ...
    # shaping
    ap.add_argument("--w-step", type=float, default=0.001)
    ap.add_argument("--w-cont", type=float, default=0.02)
    ap.add_argument("--w-play-card", type=float, default=0.02)
    ...
    for ep in range(1, args.episodes + 1):
        ...
        while not state.done and steps < args.max_steps:
            ...
            if p == 0:
                ...
                # bonus for actually removing a card from own hand
                if after_len0 < before_len0:
                    shaped_R += args.w_play_card

                # step penalty only when learner acts
        ...
        if ep % 50 == 0:
            ...
            print(
                f"ep={ep:5d} rank={rank} term={term_R:+.2f} shaped={shaped_R:+.2f} "
                f"total={R_total:+.2f} base={baseline:+.2f} mean200={mean_r:+.2f} steps={steps}"
            )
